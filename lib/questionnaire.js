 ...
 *     Περιλαμβάνει και τα `actions` (κουμπιά της φόρμας: cancel/save/submit κ.λπ.) —
 *     default τα DEFAULT_ACTIONS, όλα-ή-τίποτα (χωρίς merge ανά κλειδί).
 ...
 * Κάθε κλειδί = μία ενέργεια: { path, text, color } — path το endpoint που
 * θα χτυπήσει η φόρμα, text το λεκτικό του κουμπιού, color το Bootstrap variant.
 * Το δεσμευμένο κλειδί cancel δεν χρειάζεται path και επιστρέφει τον browser
 * στην προηγούμενη σελίδα.
 * Ισχύουν ΟΛΟΚΛΗΡΕΣ όταν ο ορισμός δεν δίνει δικό του `actions`.
 */
const DEFAULT_ACTIONS = {
  cancel: { text: "Ακύρωση", color: "secondary" },
  save: { path: "/save", text: "Προσωρινή Αποθήκευση", color: "success" },
 ...
 * αντίγραφο των DEFAULT_ACTIONS (ώστε τα instances να μη μοιράζονται το κοινό
 * object). Αν δοθεί, χρησιμοποιείται ΩΣ ΕΧΕΙ — ΧΩΡΙΣ merge ανά κλειδί με τα
 * defaults (π.χ. actions με μόνο `submit` σημαίνει φόρμα χωρίς κουμπί save)
 * και ΧΩΡΙΣ κανένα validation περιεχομένου: κάθε ενέργεια μπορεί να έχει
 * όποια keys χρειάζεται η φόρμα, και επιπλέον των path/text/color. Το
 * δεσμευμένο cancel αντιμετωπίζεται από τον browser ως ιστορική επιστροφή.
 */
function buildActions(defActions) {
  if (defActions == null)
    return Object.fromEntries(
      Object.entries(DEFAULT_ACTIONS).map(([name, action]) => [name, { ...action }])
    );
  return defActions;
}
 ...
 */
function validateAnswerSet(sectionsJsonText, answersJsonText) {
  ...
  let result = sectionsJson.every((section) => areQuestionsValid(section?.questions));
...
class Questionnaire {
  ...
  *   actions: { όνομα: { path, text, color } } — τα κουμπιά ενεργειών της
  *   φόρμας. Αν λείπει, ισχύει ΟΛΟΚΛΗΡΟ το DEFAULT_ACTIONS
  *   (cancel + save + submit).
  *   Αν δοθεί, χρησιμοποιείται ως έχει — χωρίς merge ανά κλειδί (βλ. buildActions).
...
class ResponseResults {
  ...
  _stats(questions) {
    ...
    if (stats.scorable > 0) stats.progress = stats.answered / stats.scorable;
    return stats;
  }
...
if (typeof window !== "undefined" && globalThis == window) {
  ...
  globalThis.validateAnswerSet = validateAnswerSet;
}
...
export {
    ...
    // NOT_ANSWERED_NAME,
    // COMMENT_TYPES,
    DEFAULT_ACTIONS as defaultActions,
    validateAnswerSet,
    Questionnaire,
    // Section,
    // Question,
