 ...
 *     default τα Questionnaire.defaultActions, όλα-ή-τίποτα (χωρίς merge ανά κλειδί).
...
function buildActions(defActions) {
  if (defActions == null)
    return Object.fromEntries(
      Object.entries(Questionnaire.defaultActions).map(([name, action]) => [name, { ...action }])
    );
  return defActions;
}
...
class Questionnaire {
  ...
  *   φόρμας. Αν λείπει, ισχύει ΟΛΟΚΛΗΡΟ το Questionnaire.defaultActions
  *   (cancel + save + submit).
