 ...
 */
Storage.list = async function(folderPath = '', desanitize = false) {
    ...
    for (const entry of entries) {
        ...
        if (entry.isDirectory()) {
            ...
            files.push({ name: fileName, path: relativePath, size: stat.size });
        }
    }
 ...
 */
Storage.path = async function(filePath) {
 ...
 * @returns {boolean} Αν η αποθήκευση ολοκληρώθηκε επιτυχώς
 */
