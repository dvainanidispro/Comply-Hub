...
/**
 * Makes a POST request and returns the result as JSON. Use only when server responds with JSON. 
...
Q.alert = function(message, closeText) {
    if (!Q('#qAlertDialog')) {
        ...
        dialog.innerHTML = '<p></p><div><button id="qAlertCloseButton" class="btn btn-primary"></button></div>';
...
Q.confirm = function(message, okText, cancelText) {
    if (!Q('#qConfirmDialog')) {
        ...
        dialog.innerHTML = '<p></p><div><button id="qConfirmOkButton" class="btn btn-primary"></button><button id="qConfirmCancelButton" class="btn btn-secondary"></button></div>';
        document.body.appendChild(dialog);
